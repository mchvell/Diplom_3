...
class TestProfile:
    @allure.description("Проверяем, что при переходе в ЛК без авторизации отображается страница логина")
    ...
    @allure.description("Проверяем, что после логаута отображается страница логина")
    @pytest.mark.parametrize("browser", ["chrome", "firefox"], indirect=True)
